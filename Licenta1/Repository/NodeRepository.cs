...
namespace Licenta1.Repository
{
    public class NodeRepository : GenericRepository<Node>, INodeRepository
    {
        ...
        public async void GenerateNeighbours(Node node)
        {
            ...
            var graphs = mapper.Map<List<GraphNetworkVM>>(await networkRepository.GetAllAsync());
            foreach(var item in graphs)
            {
                if(node.Neighbours!=null)
                {
                    if (name == item.Name1) node.Neighbours += "-" + item.Name2;
                    if (name == item.Name2) node.Neighbours += "-" + item.Name1;
                }
                else
                {
                    if (name == item.Name1) node.Neighbours +=  item.Name2;
